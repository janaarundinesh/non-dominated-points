...
// Used for the "C" branch of FILTER's recursive case: u in U1 and v in
// V2 already satisfy v.coords[d-1] >= u.coords[d-1] by construction (the
// split/threshold step guarantees it). So v dominates u overall iff v
// weakly covers u on the remaining coordinates 0..k-1 (k = d-1) AND u,v
// are not the exact same point (an exact duplicate has no domination
// anywhere, including on coordinate d-1, so it must be excluded even
// though it trivially "weakly covers" every coordinate).
//
// This is deliberately NOT the same as calling FILTER(U1,V2,k) again:
// FILTER requires a STRICT difference somewhere within the first k
// coordinates, but here the strict part of domination may already be
// supplied entirely by coordinate d-1 (outside this prefix), so only
// weak coverage on the prefix is required.
